...
template<typename T>
typename avl_tree<T>::box_t avl_tree<T>::build_tree_string(const avl_tree::node_t* node) const
{
    if (node == nullptr) {
        return {};
    }
...
template<typename T>
void avl_tree<T>::build_left_branch(
...
template<typename T>
void avl_tree<T>::build_right_branch(
    const box_t& box, std::size_t& gap, std::string& horizontal, std::string& vertical) const
{
    if (box.width > 0) {
...
template<typename T>
void avl_tree<T>::combine_boxes(
    avl_tree::box_lines& destination,
    const std::size_t gap_size,
    const avl_tree::box_t& left,
    const avl_tree::box_t& right) const
{
